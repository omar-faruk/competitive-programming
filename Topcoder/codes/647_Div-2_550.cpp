...
public:
	int getMaxProfit(int M, vector <int> profit, vector <int> city, vector <int> visit) {
		priority_queue<int>cityItems[M+1];
		int max=0;
		
		for(int i=0;i<int(city.size());i++){
			cityItems[city[i]].push(profit[i]);
		}
		
		for(int i=0;i<int(visit.size());i++){
			
			if(!cityItems[visit[i]].empty()){
				max+=cityItems[visit[i]].top();
				cityItems[visit[i]].pop();
			}
		}
...
int main(){
	TravellingSalesmanEasy t;
	
	
	int p[]={94,21,99,27,91,1,64,96,32,39,84,71,97,53,73,20,7,13,33,45,5,85,7,87,
...
13,29,15,99,73,72,2,81,39,75,1,54};
	int c[]={72,69,19,25,3,65,10,42,37,76,29,34,41,14,46,46,37,55,30,32,84,57,74,
...
49,54,29,17,81,83,13,8,69,5,84};
	int v[]={39,29,15,5,3,65,29,64,60,21,13,10,73,75,44,84,15,61,26,49,31,27,83,24,
...
83,35,56,82,2,14,59,68,74,32,31,3,28,38,54,38};
	...
	vector<int>city(c,c+sizeof(c)/sizeof(int));
	vector<int>visit(v,v+sizeof(v)/sizeof(int));
	
	cout<<t.getMaxProfit(85,profit,city,visit);
